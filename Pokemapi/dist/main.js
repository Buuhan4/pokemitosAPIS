...
const nextButton = document.querySelector('#next-button'); // Botón para ir a la siguiente página
const prevButton = document.querySelector('#prev-button'); // Botón para ir a la página anterior
...
function loadPokemon() {
    ...
    fetch(pokeApi)
        ...
        for (let index = 0; index < response.results.length; index++) {
            ...
            const typesResponse = yield fetch(pokemon.url); // Obtener detalles del Pokémon usando la URL
            const typesData = yield typesResponse.json(); // Convertir los datos a JSON
            const types = typesData.types.map((type) => type.type.name); // Obtener tipos de Pokémon
            ...
            pokeCardBack.innerHTML = `
                  <ul>
                      ...
                      <li><strong>Abilities:</strong> ${typesData.abilities.map((ability) => ability.ability.name).join(', ')}</li> <!-- Habilidades -->
                  </ul>
                  <ul>
            ...
            pokemonContainer === null || pokemonContainer === void 0 ? void 0 : pokemonContainer.appendChild(pokeCard); // Añadir la carta al contenedor de Pokémon
        }
